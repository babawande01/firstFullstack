...
const Navbar = () => {
  ...
  const avaterUrl = user ? `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(user.username)}` : "";
  ...
  return (
    <nav className="bg-black text-gray-200 flex justify-between items-center p-4 h-20 text-sm md:text-[15px] font-medium text-nowrap">
        <Link to={"/"}>
        <img src={Logo} alt="Logo" className='w-24 h-10 cursor-pointer brightness-125'/>
        </Link>
        ...
        <div className='flex items-center space-x-4 relative'>
            <div className='relative hidden md:inline-flex'>
                <input className ="bg-[#333333] px-4 py-2 rounded-full min-w-72 pr-10 outline-none" type="text" placeholder='Search..' /><Search className='absolute top-2 right-4 w-5 h-5'/>
            </div>
           ...
           {!user ? (<Link to={"/signin"}>
            ...
            </Link> ): (<div className='text-white'><img src={avaterUrl} alt="" className='w-10 h-10 rounded-full border-2 border-[#e50914] coursor-pointer' 
            onClick={() => setShowMenu(!showMenu)}
            />
            
            {showMenu && (
              <div className='absolute right-0 mt-2 w-64 bg-[#232323] b-opacity-95 rounded-lg z-50 shadow-lg py-4 px-3 flex flex-col gap-2 border border-[#333333]'>
